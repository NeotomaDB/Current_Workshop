...
# africa_datasets <- neotoma2::get_datasets(africa_sites, all_data = TRUE)
...
africa_datasets <- neotoma2::get_datasets(loc = africa_sf, datasettype = "pollen", all_data = TRUE)
datasets(africa_datasets) %>% 
  as.data.frame() %>% 
  DT::datatable(data = ., 
                options = list(scrollX = "100%", dom = 't'))
...
## africa_dl <- africa_east %>% get_downloads(all_data = TRUE)
## saveRDS(africa_dl, "data/dkDownload.RDS")
africa_dl <- readRDS("data/dkDownload.RDS")
```
...
There are two tables associated with entities if we want more information about them. In the future we will be adding more support for speleothems in the R package, but for now we can connect the individual entity attributes to the samples table using some SQL magic in R:
...
sisal_tx <- africa_dl %>% neotoma2::filter(datasettype == 'spelethem') %>% taxa()
```
...
sisal_tx <- africa_dl %>% neotoma2::filter(datasettype == 'spelethem') %>% taxa()
sisal_tx %>% 
  DT::datatable(data = sisal_tx, rownames = FALSE, 
                options = list(scrollX = "100%", dom = 't'))
```
