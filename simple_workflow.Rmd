...
#### Site names: `sitename="%Catemaco%"` {.tabset}

We may know exactly what site we're looking for ("Lago Catemaco"), or have an approximate guess for the site name (for example, we know it's something like "Catem", but we're not sure how it was entered specifically), or we may want to search all sites that have a specific term, for example, *Lago*.

We use the general format: `get_sites(sitename="%Catem%")` for searching by name.

PostgreSQL (and the API) uses the percent sign as a wildcard. So `"%Catem%"` would pick up ["Lago Catemaco"](<https://data.neotomadb.org/10473> for us (and picks up "Atescatempa" as well). Note that the search query is also case insensitive.

##### Code

```{r sitename, eval=FALSE}
lago_catemaco <- neotoma2::get_sites(sitename = "%Catem%")
plotLeaflet(lago_catemaco)
```
...
```{r taxa, eval = FALSE}
africa_dl <- africa_records %>% get_downloads(all_data = TRUE)
```
...
counts <- plottingSite %>%
  samples() %>%
  toWide(ecologicalgroup = c("CHAR"),
         unit = c("number"),
         elementtypes = c(">200 µm"),
         groupby = "age",
         operation = "prop") %>%
  arrange(age)
...
```{r stratiplotraw, message = FALSE, echo = FALSE}
# Get a particular site, in this case we are simply subsetting the
# `africa_dl` object to get 1 site
plottingSite <- africa_dl[[2]]
...
To plot the data we can use `rioja`'s `strat.plot()`, sorting the taxa using weighted averaging scores (`wa.order`). I've also added a CONISS plot to the edge of the the plot, to show how the new *wide* data frame works with distance metric functions.

#### Code

```{r plotStrigraphraw0, message=FALSE, warning=FALSE, out.width='90%', eval=FALSE}
# Perform constrained clustering:
counts <- counts %>% filter(if_all(-age, ~ !is.na(.)))
clust <- rioja::chclust(dist(sqrt(counts[,-1])), method = "coniss")

# Plot the stratigraphic plot, converting proportions to percentages:
plot <- rioja::strat.plot(counts[,-1] * 100, yvar = counts$age,
                  title = africa_dl[[1]]$sitename,
...
```{r plotStrigraphraw, message=FALSE, warning=FALSE, out.width='90%', echo=FALSE}
# Perform constrained clustering:
counts <- counts %>% filter(if_all(-age, ~ !is.na(.)))
clust <- rioja::chclust(dist(sqrt(counts[,-1])), method = "coniss")
